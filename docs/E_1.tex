...
\protect\hypertarget{def:pyth}{}\label{def:pyth}Es una \textbf{característica} de personas cosas u objetos que son propensos a ser medidas o cualificadas
\end{definition}
...
\begin{definition}[wwwwwww]
\protect\hypertarget{def:df1}{}\label{def:df1}wwwwwwwwwwwwwwww
\end{definition}

\begin{theorem}[wwwwwwwwwwwwwwww]
\protect\hypertarget{thm:th1}{}\label{thm:th1}wwwwwwwwwwwwwwwwwwwwwwww
\end{theorem}

\begin{example}[wwwwwwww wwww]
\protect\hypertarget{exm:ex1}{}\label{exm:ex1}wwwwwwww
\end{example}

\begin{exercise}[wwwwwwwwwwwwwwww]
\protect\hypertarget{exr:exr1}{}\label{exr:exr1}wwwwwwwwwwwwwww
\end{exercise}
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-16}{}\label{exm:unnamed-chunk-16}El espacio muestral de lanzar una monedas tres veces es \[\omega= \left\{ccc,ccs,csc,scc,css,scs,ssc,sss\right\}\] además sea \(n_c\) es número de caras y \(n_s\) el número de sellos, es posibles generar dos o mas variables aleatorias por ejemplo:
...
\protect\hypertarget{def:unnamed-chunk-17}{}\label{def:unnamed-chunk-17}Sea \(\Omega\) un espacio muestral asociado a una experimento aleatorio \(\epsilon\) y \(X\) una variable aleatoria con rango \(R_X\) definida sobre \(\Omega\). Dos eventos \(W\in\Omega\) y \(E_X\in R_X\) son \textbf{eventos equivalentes} si existe la relación
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-19}{}\label{exm:unnamed-chunk-19}La variable aleatoria discreta
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-23}{}\label{exm:unnamed-chunk-23}For a circle with the radius \texttt{r\ x},
...
\begin{example}
\protect\hypertarget{exm:unnamed-chunk-24}{}\label{exm:unnamed-chunk-24}Sea \(f(x)= \frac{\alpha}{\rho}\) es una funcion de densidad pues
\[f(x)>0,~ x\in R_X\] además
\[\int_{R_X}f(x)dx=1\]
\end{example}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-25}{}\label{exm:unnamed-chunk-25}Sea \(f(x)= \frac{\sigma}{\rho}\) es una funcion de densidad pues
\[f(x)>0,~ x\in R_X\] además
\[\int_{R_X}f(x)dx=1\]
\end{example}
...
\protect\hypertarget{def:unnamed-chunk-28}{}\label{def:unnamed-chunk-28}\[\mathbb{E}[X]=\sum _{i=1}^{n}x_{i}p(x_{i})\]
\end{definition}

\begin{definition}[Esperanza matemática de una variable aleatoria continua]
\protect\hypertarget{def:unnamed-chunk-29}{}\label{def:unnamed-chunk-29}\[\mathbb{E}[X]=\int_{-\infty }^{\infty}xf(x)dx \text{ equivalentemente }\mathbb {E}[X]=\int_{\Omega }X\,{\text{d}}P\]
\end{definition}
...
\protect\hypertarget{def:unnamed-chunk-30}{}\label{def:unnamed-chunk-30}\[F(x,y)=P[X\leq x,Y\leq y]= \sum^{x}_{u=-\infty}\sum^{y}_{v=-\infty}=p(u,v)\]
\end{definition}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-31}{}\label{exm:unnamed-chunk-31}www
\end{example}

\begin{definition}[Variable aleatoria bidmensional contínua]
\protect\hypertarget{def:unnamed-chunk-32}{}\label{def:unnamed-chunk-32}\[F(x,y)=P[X\leq x,Y\leq y]= \sum^{x}_{u=-\infty}\sum^{y}_{v=-\infty}=p(u,v)\]
\end{definition}

\begin{example}
\protect\hypertarget{exm:unnamed-chunk-33}{}\label{exm:unnamed-chunk-33}wwwww
\end{example}
...
\protect\hypertarget{def:unnamed-chunk-34}{}\label{def:unnamed-chunk-34}Sea \((X,Y)\) una variable bidimensinal discreta con rango \(R_{X\times Y}\). A cada posible resultado le asociamos un numero \[p(x,y)=P[X=x, Y=y]\] que cumple la siguientes condiciones
...
\protect\hypertarget{def:unnamed-chunk-35}{}\label{def:unnamed-chunk-35}\[F(x,y)=P[X\leq x,Y\leq y]= \sum^{x}_{u=-\infty}\sum^{y}_{v=-\infty}=p(u,v)\]
\end{definition}
